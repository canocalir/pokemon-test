...
export default class PokeDetail extends Component {
  ...
  componentDidMount() {
    fetchDetails(
      `${process.env.REACT_APP_API_BASE_ROUTE}/pokemon/${this.props.match.params.name}`
    ).then((response) =>
      this.setState({
  ...
  render() {
    const avatar =
      this.props.match.params.name === "mr-mime"
        ? `https://projectpokemon.org/images/normal-sprite/${urlCorrector(
            "mr.mime"
          )}.gif`
        : this.props.match.params.name === "nidoran-m" ||
          this.props.match.params.name === "nidoran-f"
        ? `https://projectpokemon.org/images/normal-sprite/${urlCorrector(
            this.props.match.params.name === "nidoran-m"
              ? "nidoran_m"
              : "nidoran_f"
    ...
    const pokeType = types.map((type, id) => {
      return (
        <span key={id}>
          {" "}
          {upperCaseFirstChar(type.type.name).toLowerCase()}
        </span>
      );
    });
